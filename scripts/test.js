...
async function run() {
  ...
  check('hero text has overflow guards', html.includes('.next-session-title{font-size:clamp(1.95rem,3.05vw,3rem);line-height:1.02;max-width:min(610px,62%);display:-webkit-box') && html.includes('-webkit-line-clamp:3;overflow:hidden;overflow-wrap:anywhere') && html.includes('.next-session-detail{letter-spacing:-.01em;max-width:min(780px,72%);display:-webkit-box') && html.includes('.subjects-hero-title{margin:18px 0 0;font-family:Georgia') && html.includes('.workspace-title-xl{font-family:Georgia'));
  ...
  vm.runInContext(`${subset}\nthis.__test={fmtMin,fmtSec,timeToMin,minToTime,esc,dateKey,parseLocalDate,scheduleEventOccursOnDate,getScheduleNextOccurrence,AppStats,calculateSubjectProgress};`, ctx);
  const T = ctx.__test;
