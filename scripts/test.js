...
async function run() {
  ...
  check('auth screen credits and tab indicator are polished', html.includes('class="auth-credit"') && html.includes('https://www.instagram.com/instakoti/') && html.includes('font-family:Herculanum') && html.includes('Desenvolvido por <span>@instakoti</span>') && html.includes('<div class="auth-brand-subtitle">Área de estudos</div>') && html.includes('.auth-tabs::after') && html.includes('.auth-card.signup-mode .auth-tabs::after{transform:translateX(100%);'));
  ...
  check('hero can resize shorter without breaking content', html.includes('gs-id="hero" gs-x="0" gs-y="0" gs-w="12" gs-h="6" gs-min-w="4" gs-min-h="3"') && html.includes('data-hero-density="compact"') && script.includes('function initHeroDensityObserver') && script.includes("rect.height < 245 ?'compact' : 'default'"));
  check('hero can resize narrower with compact horizontal layout', html.includes('data-hero-density="narrow"') && html.includes('#next-session-card[data-hero-density="narrow"]{min-height:200px;padding:22px 26px;grid-template-columns:1fr;gap:14px;}') && script.includes("rect.width < 560 ?'narrow'"));
  ...
  check('dashboard layout migration prevents hidden cards', html.includes('id="tasks" gs-id="tasks" gs-x="8" gs-y="6" gs-w="4" gs-h="13"') && script.includes("dashboard-layout-with-hero-pomodoro-v3") && script.includes("AppDB._set('sf_layout', DEFAULT_LAYOUT)") && script.includes('saveDashboardLayout(gsGrid.save(false))'));
  ...
  check('session annotation audio strip exists', html.includes('id="note-audio-toggle"') && html.includes('id="note-waveform"') && html.includes('id="note-audio-current"') && html.includes('id="note-ai-generate"') && !html.includes('id="note-tts-btn"'));
  check('session intelligent audio uses MediaRecorder', script.includes('MediaRecorder') && script.includes('navigator.mediaDevices.getUserMedia') && script.includes('startSessionAudioRecording') && script.includes('generateSessionAiSummary') && script.includes('session-audios'));
  check('session intelligent card keeps lean metadata', html.includes('Resumo inteligente') && html.includes('O resumo gerado pela IA aparecer') && !html.includes('id="note-meta-session"') && html.includes('.session-note-meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))'));
  check('session AI button has loading feedback', html.includes('.session-note-ai-btn.is-loading') && html.includes('@keyframes sessionSpin') && script.includes("aiBtn.classList.toggle('is-loading'"));
  check('session intelligent card adapts to narrow resize', html.includes('data-note-density="tiny"') || (html.includes('[data-note-density="tiny"]') && script.includes('ResizeObserver') && script.includes('initSessionNoteDensityObserver')));
  check('session intelligent sections do not collapse vertically', html.includes('.session-note-header,.session-note-meta,.session-note-audio,.session-note-section{flex:0 0 auto;}'));
  check('session AI results can be removed without deleting audio flow', html.includes('session-note-ai-remove') && script.includes('function removeSessionAiResult') && script.includes('suggested_review = {}') && script.includes('clearAudioFile') && script.includes('const hasAiContent'));
  check('session insight removal syncs editable note and AI source', script.includes('function syncRemovedSessionInsight') && script.includes('getSessionInsightComparableText') && script.includes('DataStoreSupabase.audioNotes.update(cloudNote.id, {insights:nextInsights})') && script.includes('removeSessionInsight(button.dataset.insightId).catch') && script.includes('getSessionInsightComparableText(item) !== removedText'));
  check('session AI reload prefers completed results over newer drafts', script.includes('order=created_at.desc&limit=12') && script.includes("item.status === 'completed' && hasAiPayload(item)") && script.includes('|| rows.find(item=>item.audio_path)'));
  check('session AI extras survive Supabase note pull', script.includes('function restoreSessionAiResultFromCloudIfNeeded') && script.includes('noteHasSessionAiExtras(note)') && script.includes('applySessionAiResult(buildSessionAiResultFromCloudNote(cloudNote), {silent:true})'));
  ...
  check('Supabase intelligent audio storage is modeled safely', schema.includes('create table if not exists public.session_audio_notes') && schema.includes('references public.study_sessions(user_id, id)') && schema.includes("status in ('draft', 'uploaded', 'processing', 'completed', 'failed')") && schema.includes("insert into storage.buckets") && schema.includes("'session-audios'") && schema.includes('Users can manage own session audio notes') && schema.includes('Users can read own session audios'));
  check('AI endpoint stays backend-only', !html.includes('GEMINI_API_KEY') && !html.includes('generativelanguage.googleapis.com') && script.includes("callFunction('analyze-session-audio'") && fs.existsSync(path.join(root, 'supabase', 'functions', 'analyze-session-audio', 'index.ts')));
  check('local env files are ignored', gitignore.includes('.env.local') && gitignore.includes('.env.*') && !gitignore.includes('!.env.local'));
  ...
  check('dashboard greeting uses branded popup for preferred name', html.includes('id="modal-display-name"') && html.includes('display-name-modal-card') && html.includes('Como você gostaria de ser chamado?') && script.includes('function openPreferredDisplayNameModal') && script.includes('function submitPreferredDisplayName') && script.includes("on('workspace-title-heading','click'") && script.includes("on('btn-display-name-save','click'") && script.includes('DataStoreSupabase.profiles.updateDisplayName') && !script.includes('window.prompt('));
  check('dashboard layout has reliable cloud sync points', script.includes('function saveDashboardLayout') && script.includes("DataStore.cloudWrite('preferências do painel'") && script.includes('saveDashboardLayout(gsGrid.save(false), {sync:true})') && script.includes('saveDashboardLayout(gsGrid.save(false));'));
